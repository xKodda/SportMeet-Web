...
const HeroSection = () => {
  const { trackAppDownload } = useGoogleAnalytics();

  const handleAppDownload = (platform: 'ios' | 'android') => {
    trackAppDownload(platform);
    // Aquí puedes agregar la lógica de redirección a las tiendas
    if (platform === 'ios') {
      // window.open('https://apps.apple.com/app/sportmeet', '_blank');
    } else {
      // window.open('https://play.google.com/store/apps/details?id=com.sportmeet.app', '_blank');
    }
  };
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
      ...
      <div className="relative z-10 text-center px-6 max-w-6xl mx-auto">
        ...
        <motion.p
          ...
          className="text-2xl md:text-3xl text-white mb-16 font-light max-w-2xl mx-auto leading-relaxed drop-shadow-lg"
        >
          Encuentra con quién jugar.
          <br />
          <span className="text-lg text-gray-200">Conecta. Juega. Comparte.</span>
        </motion.p>
        ...
        >
          ...
          >
            <motion.div 
              ...
              whileTap={{ scale: 0.95 }}
              className="bg-black/90 backdrop-blur-sm rounded-xl px-8 py-3 flex items-center space-x-3 outline-none border border-white/20 shadow-2xl hover:shadow-3xl transition-all duration-300"
            >
              {/* Apple Logo */}
